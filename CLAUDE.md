...
- **Reminder: `scripts/xg_pred_vs_actual_validation.py` (adăugat 2026-08-10) — de re-rulat peste 2-3 săptămâni, eșantion azi prea mic pentru o decizie.** Compară xG-ul propriu al Oracle (`home/away_xg_pred`, formulă din `feature_engine.calibrate_xg()`) cu xG-ul real, bazat pe șuturi (`home/away_xg_actual`, colectat deja de Foundation Data Layer din Flashscore + soccerfootballinfo — nimic nou de colectat). Rezultat preliminar (n=35 meciuri, 2026-08-10): corelație aproape zero pe partea de casă (0.019) vs. moderată pe oaspete (0.550), MAE agregat 0.747 — asimetrie de urmărit, NU o concluzie (eșantion prea mic per ligă, 8-15 meciuri). Dacă tiparul se confirmă pe eșantion mai mare, ar justifica un experiment de recalibrare separat (niciodată automat — regula din secțiunea „Filosofia proiectului" de mai sus).
  - **Extensie propusă, NEconstruită încă (2026-08-10)**: adăugarea Understat ca a treia coloană de comparat (Oracle vs. Flashscore/SFI vs. Understat), accesibil live prin conectorul Exa (`mcp__Exa__web_fetch_exa` — ocolește blocajul de rețea al sandbox-ului pe `understat.com`, confirmat live pe un meci real, date server-randate, citite curat). Verificat însă că Understat acoperă STRICT 6 ligi (EPL, La Liga, Bundesliga, Serie A, Ligue 1, RFPL) — confirmat din documentația oficială + un proiect open-source de referință. Verificare live (2026-08-10): 0 meciuri din aceste 5 ligi (RFPL neurmărită) au azi ambele semnale (`xg_pred`+`xg_actual`) simultan — deși `xg_actual` există pentru zeci de meciuri per ligă (15-38), `xg_pred` e aproape complet gol (0-6) — pentru că se scrie DOAR când o predicție chiar a fost cerută/vizualizată live în aplicație înainte de meci, nu calculat în masă. Nu se construiește Understat până nu există un eșantion real de testat — ar sta cod netestat, fără sens. Revino la asta când `xg_pred` începe să se populeze pentru Premier League/La Liga/Bundesliga/Serie A/Ligue 1 (accelerabil dacă predicțiile pentru aceste ligi sunt vizualizate în aplicație înainte de meci).
