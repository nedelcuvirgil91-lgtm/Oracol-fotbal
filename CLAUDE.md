...
- **`night_sync.yml` rulează cu orele când cota `oddsapi` e epuizată — de optimizat, nu acum** (raportat de proprietarul produsului, 2026-08-21, log real din rularea manuală declanșată pentru ADR-058: peste 10.500 de linii `[RateLimit] oddsapi: cota zilnica epuizata (confirmat din header), cererea e blocata.`, toate cu același timestamp). Verificat în cod: `RateLimitManager.can_request()` (`rate_limit_manager.py:156-169`) blochează CORECT orice cerere HTTP reală după epuizarea cotei (fail-safe funcțional, zero risc de bani/cotă suplimentară cheltuită) — dar `RequestManager.should_request()` (`request_manager.py:82-88`, apelat din `oracle_api.py:318`/`:392`) e verificat și loghează un `WARNING` separat la FIECARE tentativă individuală (per ligă/zi/fixture), fără niciun scurt-circuit la nivel de etapă care să sară peste tot restul lucrului legat de cote odds odată ce se confirmă epuizarea zilei. Rezultat: mii de iterații Python + I/O de logging inutile, repetate identic. **NEVERIFICAT încă**: cât din durata totală de „ore" a `night_sync` provine efectiv din acest tipar (vs. alte etape genuin lente) — nu s-a măsurat, doar s-a observat volumul de log. De rezolvat: scurt-circuit la nivel de orchestrare (verifică o dată `can_request("oddsapi")` la începutul etapei de odds, sari peste toată etapa dacă e epuizată, nu per-item) + măsurare reală a duratei per etapă înainte de orice optimizare (Regula „Verificat, nu presupus").
