-- =============================================================================
-- Football Oracle — Migration 038: Season Model (Foundation Data Layer)
-- =============================================================================
-- "Raspuns oficial - Foundation Data Layer (clarificari finale)", pct. 1:
-- season devine parte din modelul canonic. Format text liber, asa cum il
-- ofera providerul (ex. "2026-2027") - NU se deriva din reguli calendaristice
-- proprii (interzis explicit). Daca providerul nu ofera sezonul, coloana
-- ramane NULL - nicio stare necunoscuta aproximata (North Star #8).
--
-- Scop: pregatire pentru ponderarea Oracle/ML pe sezon (viitor, neimplementat
-- aici) SI pentru Season Cleanup (pct. 2/3 - infrastructura DOAR, FARA
-- stergere - vezi providers/flashscore/season_cleanup.py, dry-run only).
--
-- Pur aditiv (ADD COLUMN IF NOT EXISTS + CREATE OR REPLACE FUNCTION),
-- idempotent, nicio scriere/migrare de date.
-- =============================================================================

ALTER TABLE match_history ADD COLUMN IF NOT EXISTS season TEXT;
ALTER TABLE match_statistics_extended ADD COLUMN IF NOT EXISTS season TEXT;
ALTER TABLE player_match_stats ADD COLUMN IF NOT EXISTS season TEXT;
ALTER TABLE player_match_stats_extended ADD COLUMN IF NOT EXISTS season TEXT;
ALTER TABLE flashscore_match_context ADD COLUMN IF NOT EXISTS season TEXT;
ALTER TABLE flashscore_standings_snapshot ADD COLUMN IF NOT EXISTS season TEXT;
ALTER TABLE flashscore_raw_extraction ADD COLUMN IF NOT EXISTS season TEXT;
ALTER TABLE flashscore_data_completeness ADD COLUMN IF NOT EXISTS season TEXT;

-- RPC canonic (upsert_match_canonical) - extindere aditiva (aceeasi forma ca
-- migratia 036, DOAR + season COALESCE la UPDATE, + season la INSERT).
