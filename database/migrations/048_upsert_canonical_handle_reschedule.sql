-- ============================================================================
-- Migrarea 048 — upsert canonic: trateaza meciurile REPROGRAMATE
-- ============================================================================
-- Context: `_upsert_match_canonical_locked` cauta randul existent dupa cheia
-- naturala (home_team, away_team, kickoff_date). Cand un provider muta un meci
-- pe alta data, lookup-ul cu data NOUA nu gaseste randul (are data VECHE),
-- functia cade pe ramura INSERT, iar INSERT-ul violeaza
-- `idx_match_history_fixture` (UNIQUE pe fixture_id) — exceptie, iar intregul
-- pas de sincronizare esueaza.
--
-- Impact real masurat: `flashscore_weekly_fixtures.yml` (SINGURUL workflow
-- care descopera meciuri VIITOARE) a esuat pe 17 si 20 august 2026 pe exact
-- acelasi meci — Celta Vigo - Osasuna, mutat de pe 2026-08-16 pe 2026-08-27.
-- Ultima rulare reusita: 13 august. Consecinta: nicio descoperire de fixture
-- nou timp de 10 zile (Europa/Conference League fara meciuri dupa 20 august).
--
-- Fix: inainte de a decide INSERT, se cauta un rand LIVE cu acelasi
-- fixture_id. Daca exista si are alta data, i se actualizeaza data (meciul e
-- acelasi, doar reprogramat), iar fluxul continua pe ramura UPDATE obisnuita.
--
-- Garda: daca data noua e deja ocupata de ALT rand live, se intoarce
-- `hard_conflict` cu motiv explicit — rand nescris, vizibil, niciodata pierdut
-- tacit (ID-025-01/03). Nu se forteaza o scriere care ar viola
-- `idx_match_history_natural_key_canonical`.
--
-- Idempotenta: CREATE OR REPLACE; re-rularea nu schimba nimic.
-- Restul corpului functiei e IDENTIC cu migrarea 026 — acest fisier a fost
...
BEGIN
  ...
  IF v_fixture IS NOT NULL THEN
    SELECT id, left(kickoff_date, 10) INTO v_resched_id, v_old_date
    FROM match_history
