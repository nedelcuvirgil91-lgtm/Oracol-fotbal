-- ============================================================================
-- Migrarea 049 — upsert canonic: identitate contradictorie pe acelasi fixture_id
-- ============================================================================
-- Context: migrarea 048 a tratat cazul "acelasi fixture_id, alta DATA"
-- (meci reprogramat). Rularea din 2026-08-23 a expus un al doilea caz, cu
-- exact acelasi efect distructiv: "acelasi fixture_id, alte ECHIPE".
--
-- Caz real: Ligue 1, mid=p2AX2W4D. Flashscore a extras acelasi meci ca
-- "PSG - Rennes" pe 31 iulie si ca "Rennes - PSG" pe 23 august. Verificat
-- extern: se juca la Roazhon Park, deci Rennes era gazda si extragerea din
-- iulie era gresita. Cauza radacina (orientarea dedusa din ordinea DOM) e
-- reparata separat, in providers/flashscore/normalizer.py.
--
-- Fara acest bloc, cheia naturala nu gaseste randul existent, functia cade pe
-- INSERT, iar INSERT-ul violeaza idx_match_history_fixture (UNIQUE) ->
-- exceptie, si intregul pas de sincronizare esueaza. 390 din 393 de meciuri
-- reusite, dar workflow-ul rosu.
--
-- Decizie deliberata: NU se alege automat un castigator intre doua afirmatii
-- de identitate contradictorii. Care echipa e gazda determina ELO, forma,
-- H2H, atribuirea xG pe parti si avantajul terenului propriu — e o decizie de
-- identitate, verificata de om (regula D3, precedent Nijmegen/Vitesse), nu una
-- mecanica. Se intoarce hard_conflict: rand NESCRIS, vizibil, trasabil.
--
-- Ordinea verificarii e deliberata: INAINTEA UPDATE-ului de reprogramare din
-- 048. Altfel am muta data unui rand si am raporta apoi scrierea ca esuata,
-- lasand o modificare partiala in urma.
--
-- Idempotenta: CREATE OR REPLACE; re-rularea nu schimba nimic.
-- Restul corpului functiei e IDENTIC cu migrarea 048 — acest fisier a fost
-- generat programatic din ea, ca sa nu existe risc de transcriere manuala.
-- ============================================================================
