...
- **Stage 2** (opțional, operațional, NU parte obligatorie): curățare one-time
  a celor ≤29 rânduri prezise-nefinalizate (`UPDATE ... SET (cele 10) = NULL
  WHERE prob_home_pred IS NOT NULL AND actual_result IS NULL`), cu SQL arătat
  înainte de rulare (Supabase-safety). D3.5 funcționează corect și fără el.
...
2. ⬜ Stage 1 — `_cache_prediction` nu mai trimite cele 10 `FEATURE_COLUMNS`.
3. ⬜ Stage 3 — eliminare scriere legacy `actual_*` + gărzi AST.
4. ⬜ Stage 2 — curățare ≤29 rânduri, DOAR dacă solicitat explicit ulterior.
