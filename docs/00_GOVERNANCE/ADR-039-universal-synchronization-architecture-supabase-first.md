...
5. **Migrare provider-cu-provider, niciodată big-bang.** Secvențiere pe risc (audit §6, **corectată post-R-Sync-2** — vezi audit §6b pentru dovada/justificarea corecției): API-Football injuries/coaches (**finalizat, R-Sync-2**) → football-data.org formă/standings, **exclus explicit fixtures** (R-Sync-3) → TheSportsDB stats+ELO, exclus fixtures (R-Sync-4) → Weather (R-Sync-5) → FreeLF formă/standings + Odds API fallback H2H/formă, exclus explicit orice rol de descoperire (R-Sync-6) → **Universal Match Discovery Layer** (R-Sync-7, etapă proprie, ultima înaintea curățării finale — TOȚI providerii de fixtures într-un singur pas: FreeLF, Odds API, football-data.org, ESPN, TheSportsDB, **și API-Football** — niciun provider de fixtures nu rămâne tratat separat sau privilegiat, inclusiv API-Football, care nu e un caz special doar pentru că injuries/coaches ale lui au migrat deja) → eliminarea finală a `self.api` din Oracle Engine (R-Sync-8).
