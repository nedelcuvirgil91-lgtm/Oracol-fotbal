...
5. **Migrare provider-cu-provider, niciodată big-bang.** Secvențiere pe risc (audit §6): API-Football → football-data → ESPN → TheSportsDB → Weather → FreeLF (formă) → Odds API (fallback) → descoperirea meciurilor (etapă proprie, ultima, cea mai mare).
