...
**Două găsiri reale, de la validarea live, nu prezumate — AMBELE reparate imediat, migrarea 025, aprobare explicită proprietar produs** (nu amânate pentru G3/G4 — „infrastructura de guvernanță trebuie să fie complet deterministă", „asta mută validarea acolo unde trebuie: în baza de date"):

1. **Ambiguitate de tie-break în `current_health`**: cele 3 rânduri eligibile din demonstrație au `evaluated_at` identic (aceeași instrucțiune INSERT) — `DISTINCT ON (gate_key, entity) ... ORDER BY evaluated_at DESC` nu avea un al doilea criteriu determinist pentru egalitate. **Reparat**: `ORDER BY ... evaluated_at DESC, id DESC` (migrarea 025, `CREATE OR REPLACE VIEW`).
2. **Schema accepta silențios date imposibile** — testat explicit: `INSERT` cu `live_count=10, matched_count=300` reușea fără nicio eroare. **Reparat**: trei `CHECK` constraints noi (migrarea 025) — toate câmpurile de cardinalitate `>= 0`, `matched_count <= live_count AND matched_count <= scheduled_count`, `equivalence_score` (când nu e NULL) în `[0, 1]`. **Re-testat live, după aplicare**: exact același `INSERT` (live_count=10, matched_count=300) → `ERROR 23514: violates check constraint "equivalence_evaluations_matched_within_bounds"`, tranzacție respinsă, 0 rânduri scrise (confirmat prin `SELECT count(*)`).

Cleanup confirmat: `DELETE FROM equivalence_evaluations WHERE gate_key='R-Sync-7b-demo'` (5 rânduri, inclusiv cazul invalid original), apoi `SELECT count(*)` → 0, atât pentru `gate_key='R-Sync-7b-demo'` cât și pentru tabela întreagă — confirmat din nou după migrarea 025 (0 rânduri, inclusiv încercarea de INSERT invalid re-testată, care de această dată n-a mai scris nimic pentru că a fost respinsă la nivel de bază de date).
