...
## Addendum 5 (2026-07-29) — REVIZUIRE Addendum 4: `actual_result` devine writer secundar Flashscore, nu doar `actual_home_goals`/`actual_away_goals`

**Context**: audit complet, live, al infrastructurii (cerut explicit de proprietarul produsului, "Vreau un audit complet al infrastructurii implementate... Rulează validări end-to-end") — validare reală prin GitHub Actions (`flashscore_foundation_data_layer.yml`, `workflow_dispatch`, ref pe branch-ul de lucru) + interogări read-only Supabase (`mcp__Supabase__execute_sql`, proiectul `Prediction`, `gtlpyxzocacaqyompkwe`).

**Descoperire**: Discovery a găsit meciuri reale pe toate cele 9 competiții urmărite (inclusiv cele 5 adăugate în Faza 2 la Nivel B de verificare — confirmate acum live, pe fixture-uri reale, Nivel A). O rulare LIVE (nu dry-run) pentru Romania SuperLiga + Champions League a persistat 2 meciuri Champions League noi cu succes (Delta Sync a sărit corect cele 2 SuperLiga deja colectate). Verificare directă în `match_history`: `league="Champions League"` (nu `"UEFA Champions League"` — bug-ul din Faza 2 §5 confirmat corectat live).

**Bug critic găsit prin aceeași verificare**: `actual_result` era `NULL` pe **17/17** meciuri Flashscore colectate vreodată (toate sesiunile), deși `actual_home_goals`/`actual_away_goals` erau populate corect pe toate. Cauza: Addendum 4 restrânsese `actual_result` EXCLUSIV la `sync/sync_results.py`. Dar `sync_results.py` nu acoperă azi Romania SuperLiga (gol documentat explicit în CLAUDE.md, „Goluri cunoscute") și nici calificările UEFA (fără sursă API dedicată) — **exact competițiile pentru care Flashscore există**. Consecință: gate-ul `actual_result IS NOT NULL`, folosit de FIECARE query Team DNA (`get_team_recent_advanced_stats`, `get_team_recent_statistics_extended`, `get_team_recent_player_ratings`, `get_team_recent_match_events`, etc.) nu se satisface NICIODATĂ pentru un meci colectat de Flashscore — Team DNA rămânea gol prin construcție, indiferent cât de complete erau celelalte date.

**Decizie revizuită**: Flashscore devine writer secundar autorizat și pentru `actual_result`, nu doar pentru `actual_home_goals`/`actual_away_goals`. Derivarea e IDENTICĂ, literă cu literă, cu formula deja folosită de `sync/sync_results.py` (`"H" if home>away else "A" if home<away else "D"`) — zero ambiguitate, scrisă DOAR dacă ambele goluri sunt prezente (niciodată o stare parțială ghicită). Scrierea trece prin ACELAȘI RPC COALESCE-safe + hard-conflict (`upsert_match_canonical`) — verificat direct în migrația 036: `_upsert_match_canonical_locked` acoperă deja explicit `actual_result` în COALESCE și în detecția de `hard_conflict`, deci **niciun cod de infrastructură nou, nicio migrare nouă** — doar `normalizer.py` a fost extins să includă `actual_result` în dict-ul returnat.

**De ce nu a fost prins la Addendum 4**: acea decizie a fost luată fără o rulare live reală (aplicația nu avea încă meciuri colectate cu care să se testeze consecința gate-ului). Motivația inițială („nicio ambiguitate de interpretare a scorului brut extras") era despre `actual_home_goals`/`actual_away_goals` (extragere de text), nu despre `actual_result` (derivare aritmetică din perechea deja extrasă) — o distincție care a scăpat la momentul deciziei inițiale.

**Implementare**:
- `providers/flashscore/normalizer.py` — `normalize_match_statistics()` calculează acum `actual_result` din `actual_home_goals`/`actual_away_goals` deja extrase, `None` dacă oricare lipsește.
- `tests/test_canonical_feature_ownership.py` — garda AST pozitivă extinsă să confirme și `actual_result` (elimină garda negativă anterioară, care interzicea exact acest lucru).
- 3 teste noi de comportament (`tests/test_providers_flashscore_normalizer.py`): derivare H/A/D pe fixture real (1-1 → D), derivare pentru toate cele 3 cazuri (monkeypatch pe `_extract_final_score`), degradare la `None` fără ambele goluri.
- Validat live: următoarea rulare reală (după acest fix) va popula `actual_result` pentru meciuri noi; cele 17 rânduri deja colectate rămân cu `actual_result=NULL` până la un backfill SQL separat (`UPDATE match_history SET actual_result = CASE ... WHERE fixture_id LIKE 'flashscore_%' AND actual_result IS NULL AND actual_home_goals IS NOT NULL AND actual_away_goals IS NOT NULL`), care necesită confirmarea explicită a proprietarului produsului înainte de rulare (scriere directă pe date live, skill `supabase-safety`) — nu executat unilateral în acest audit.

**Consecințe**: Team DNA poate acum, real, să se populeze pentru meciurile Flashscore colectate DE ACUM ÎNAINTE — gate-ul care îl bloca silențios e eliminat la sursă. Cele 17 rânduri deja colectate rămân needisponibile pentru Team DNA până la backfill-ul opțional de mai sus.
