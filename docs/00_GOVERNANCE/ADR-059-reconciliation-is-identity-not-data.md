...
Descoperit în timpul execuției ADR-060 Faza 2c (reconciliere de masă, 2.827 rânduri, declanșată de extinderea vocabularului) — dar **nu prin manifestarea reală a bug-ului**. Corecție de precizie, importantă: `check_identity_drift.py`, rulat imediat după reconcilierea de masă, a ieșit roșu (exit 1) — corect — din cauza singurului conflict HARD CONFLICT nou (`nijmegen||vitesse||2023-10-01`, investigat separat, discrepanță reală de scor între surse, fără bug cunoscut care s-o explice). Restul listei afișate în acel raport (sute de chei, ex. „young boys||inter milan", „real madrid||ac milan") era secțiunea **informativă** „ℹ️ grupuri din baseline care nu mai apar (probabil deja reconciliate)" — dovada CORECTĂ că reconcilierea de masă își făcuse treaba, nu o alarmă. A fost citită greșit, la prima trecere, ca o listă de „grupuri NOI" (secțiunea 🔴), ceea ce a declanșat o investigație pe o pistă falsă — verificarea directă în bază a unui exemplu din acea listă (Real Madrid–AC Milan) a confirmat exact starea așteptată (un singur rând live, zero duplicat), nu un bug.

Investigația greșit direcționată a dus totuși la o descoperire reală, prin citirea directă a codului `_fetch_key_index()` (nu prin observarea vreunui efect): **cauza radacină**: paginarea prin `.range()` **fără `.order()` explicit**. PostgREST/Postgres nu garantează ordine stabilă între cereri succesive fără `ORDER BY` — sub scriere concurentă pe tabelă, același rând fizic ar putea fi întors de două ori, în două pagini diferite. Defectul e real și verificat independent (secțiunea de mai jos), dar **nu a produs valul de "grupuri noi" observat inițial** — acela nu a existat niciodată; a fost o eroare de citire a raportului, corectată aici explicit, conform regulii „verificat, nu presupus".
