...
| `main` HEAD | `75378f9` — fast-forward de pe `claude/continua-faza-1-adr5-o52jat`, `git push origin main`, 2026-08-03 (**Pasul 7 EPIC ML Activation, ÎNCHIS COMPLET** — redenumire `dna_weight`→`base_weight` executată conform ADR-047: 12 fișiere de cod redenumite, `weights.json` actualizat, **migrare Supabase `model_weights` confirmată live** — `SELECT` post-`UPDATE` arată `base_weight` prezent/`dna_weight` absent, global + toate cele 11 ligi, `updated_at=2026-08-03 22:09:20`. Verificare funcțională: `resolve_league_weights()` + `calibrate_xg()` pe date live produc predicție identică cu calculul pe `weights.json` local. Descoperire în timpul implementării — `recalibration_log.new_dna_w` (coloană SQL live, 0 rânduri) — decisă explicit **out of scope** de proprietarul produsului, documentată în ADR-047 §10, `supabase_client.py` neatins, conform noii Discovery Rule) |
