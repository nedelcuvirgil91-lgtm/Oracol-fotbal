...
| `main` HEAD | `52db6e7` — fast-forward de pe `claude/continua-faza-1-adr5-o52jat`, `git push origin main`, 2026-08-04 (**Pasul 9 EPIC ML Activation, ÎNCHIS** — ADR-048 aprobat: golul critic de persistență a modelului ML e rezolvat. `LearningAlgorithm` capătă `get_trained_model()` (`xgboost_v1` expune modelul real; `production_champion`/`league_weights_adaptive` întorc `None`, ADR-028). `continuous_learning._phase_b_train_new()` apelează acum `model_artifact_storage.save_model_artifact()` din Orchestrator, imediat după antrenare reușită, ÎNAINTE de `challenger_manager.create_challenger()` (D1/D2, ADR-048) — dacă persistarea eșuează (model lipsă sau storage, inclusiv excepție neașteptată), Challenger-ul NU se creează deloc (D3), garantând invariantul de sistem INV-1: „niciun Challenger fără artefact confirmat". `docs/04_LEARNING_CORE/MODEL_ARTIFACT_STORAGE_CONTRACT.md` §3 amendat pentru a reflecta fluxul real (fără pas `compare()` separat pre-creare, divergent de designul inițial scris înainte de ADR-016/ADR-030). `training_runner.py`, `challenger_manager.py`, `promotion_service.py`, `champion_loader.py`, `challenger_shadow.py` — neatinse. `champion_loader`/`promotion_service`/`challenger_shadow`, deja scrise corect dar mereu eșuate din lipsă de artefact, pot acum funcționa efectiv — primul scriitor real al `model-artifacts` există. `pytest tests/` — 1968 passed, 2 skipped (3 eșecuri preexistente în `test_oracle_api_tsdb_per_league_gate.py`, confirmate identice pre-Pasul 9 via `git stash`, neatinse de acest task) |
