...
| `main` HEAD | `c297a80` — fast-forward de pe `claude/continua-faza-1-adr5-o52jat`, `git push origin main`, 2026-08-03 (include audituri Etapa 1-3, `ML_ACTIVATION_IMPLEMENTATION_PLAN.md` post Architecture Review, Pașii 1-2 finalizați + **Pasul 3 finalizat** EPIC ML Activation — eliminare cod mort `injury_manager.py`: `get_injury_report_from_cache()`, `ABSENCE_CERTAINTY`, `_impact_label()` bazat pe minute, `MIN_STARTER_MINUTES`/`MIN_KEY_PLAYER_MINUTES`, toate confirmate fără apelanți prin grep exhaustiv, calea live neatinsă) |
