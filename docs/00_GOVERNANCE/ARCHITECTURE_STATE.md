...
| `main` HEAD | `07894e5` — fast-forward de pe `claude/continua-faza-1-adr5-o52jat`, `git push origin main`, 2026-08-03 (include audituri Etapa 1-3, `ML_ACTIVATION_IMPLEMENTATION_PLAN.md` post Architecture Review, Pașii 1-6 finalizați EPIC ML Activation, plus **ADR-047 APROBAT** — redenumire `dna_weight`→`base_weight`, cu Deployment Rule atomică obligatorie (cod + `weights.json` + `model_weights` Supabase migrate în aceeași sesiune, fără stări intermediare, fără shim de compatibilitate) — plus formalizarea Definition of Done pentru tot EPIC-ul (`ML_ACTIVATION_IMPLEMENTATION_PLAN.md` §7) și a Vision Statement-ului permanent Oracle/ML (`CLAUDE.md`, secțiune nouă). Implementarea efectivă a Pasului 7 (redenumirea propriu-zisă) urmează, ca task separat) |
