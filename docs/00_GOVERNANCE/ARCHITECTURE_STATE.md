...
| `main` HEAD | `cd5d45e` — fast-forward de pe `claude/continua-faza-1-adr5-o52jat`, `git push origin main`, 2026-08-04 (**Pasul 10a EPIC ML Activation, ÎNCHIS** — ADR-049 (ACCEPTED) implementat parțial: Temperature Scaling ca strat de calibrare post-hoc pentru probabilitățile ML, antrenat pe setul out-of-fold al walk-forward-ului (`ml_predictor._fit_temperature()`), cu gardă explicită — T optimizat acceptat doar dacă bate strict log-loss-ul de baseline la T=1.0, altfel se păstrează T=1.0 (evită calibrarea pe zgomot statistic). Persistare prin artefact SEPARAT (`learning_core/calibration_artifact_storage.py`, `<training_run_id>.calibration.json`), independent de `model_artifact_storage.py` (neatins), derivat din același `training_run_id` prin import read-only al helper-ului de naming. `continuous_learning._phase_b_train_new()`: persistarea calibrării devine gating la fel ca persistarea modelului (extensie INV-1/ADR-048) — Challenger nu se creează fără calibrare persistată cu succes; eșec parțial lasă ambele artefacte orfane, fără cleanup ad-hoc. `challenger_shadow.predict_with_challenger()`: aplicarea calibrării la predicția shadow e degradare grațioasă (fallback la `predict_proba()` nativ), nu gating. Vezi §3.1 pentru detalii complete și scope-ul explicit rămas (Pasul 10b — `champion_loader.py`/`oracle_engine.py`, NEATINSE în acest pas). `pytest tests/` — 2008 passed, 2 skipped (aceleași 3 eșecuri preexistente în `test_oracle_api_tsdb_per_league_gate.py`, neatinse) |
...
- **Step 10b: ⏳ PLANNED** (neînceput) — wiring-ul de servire live: `champion_loader.py` (încărcarea calibrării Champion-ului activ) + `oracle_engine.py` (`_resolve_champion()`, aplicarea calibrării la predicția servită efectiv utilizatorului). Până la implementarea acestui pas, **Champion-ul live continuă să servească probabilitățile brute, necalibrate** — calibratorul există complet în pipeline-ul de învățare/evaluare (antrenare + shadow), dar nu afectează încă nicio predicție servită live. `docs/04_LEARNING_CORE/RUNTIME_CONTRACT.md` (Frozen, 6 condiții de utilizabilitate a Champion-ului) rămâne neatins — confirmat explicit că nu necesită o a 7-a condiție, calibrarea fiind gândită ca degradare grațioasă și la Step 10b, nu ca blocaj.

Fișiere confirmate neatinse în Step 10a (`git diff --stat`, gol): `champion_loader.py`, `oracle_engine.py`, `promotion_service.py`, `training_runner.py`, `challenger_manager.py`, `model_artifact_storage.py`, `RUNTIME_CONTRACT.md`.
