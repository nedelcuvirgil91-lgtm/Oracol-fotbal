...
| `main` HEAD | `a603a61` — fast-forward de pe `claude/continua-faza-1-adr5-o52jat`, `git push origin main`, 2026-08-03 (**Pasul 8 EPIC ML Activation, ÎNCHIS — cod NEATINS, decizie deliberată**: bugul de dublă numărare `avg_goals_for` în `compute_team_offdef_rating()` a fost investigat complet — demonstrat cu backtest walk-forward pe 4849 de meciuri, eliminarea lui simplă degradează Oracle, un candidat de compensare (`goals_weight` 0.45→0.75) a fost testat și arată rezultate egale/mai bune pe toate cele 5 metrici cerute — **dar recalibrarea NU a fost aplicată**: proprietarul produsului a decis explicit că bug fix-urile nu introduc schimbări de model, orice modificare a parametrilor matematici Oracle e experiment de calibrare separat, cu propria aprobare. `feature_engine.py`/`oracle_engine.py`/`weights.json` rămân identice cu starea de la Pasul 7 (`8f85416`), zero schimbare de comportament al predicției. Investigația e documentată integral în `docs/03_ENGINE/ORACLE_INSIGHT_GOALS_WEIGHT.md` ca „Oracle Insight"; recalibrarea candidat rămâne **FOLLOW-UP-P8-01 — Oracle Calibration**, task separat, neînceput. Regulă nouă permanentă adăugată în `CLAUDE.md` — bug fix-urile nu introduc schimbări de model) |
