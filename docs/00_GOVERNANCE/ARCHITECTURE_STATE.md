...
**Actualizare punctuală, 2026-09-06**: §4 („Stare canonică relevantă") a fost **remăsurat integral** — valorile datate 2026-07-28 erau false în toate punctele care contau și au indus în eroare o sesiune ulterioară. Faptele noi: primul campion REAL din istoria proiectului (`blend_v1/all`, promovat 2026-08-23, prin ciclul complet cu aprobare umană), Feedback Loop end-to-end ÎNCHIS (§5 punctul 3, tăiat), și blocajul curent al antrenării ML (ambele sloturi de Challenger ocupate de Challengeri în `monitoring`, zero antrenări noi din 24 august — ADR-057 PROPUS, nedecis). Tot azi, `shadow_testing.get_shadow_predictions()` a fost reparat (paginare; evaluarea rula pe un subeșantion trunchiat la 1000 de rânduri) — `main` @ `4a4b206`.
...
| `validation_analysis.yml` (ADR-052 §2.4) | zilnic `0 10 * * *` + săptămânal (luni) `15 10 * * 1` | `run_validation_analysis.py --cadence daily\|weekly` | `validation_analysis_enabled` — **`true` în producție, verificat 2026-09-06** (textul „implicit `False`" descria valoarea implicită din cod, nu starea live; produce rapoarte reale în `validation_analysis_reports`, 19 zilnice + 2 săptămânale) |

**Stare canonică relevantă — REMĂSURAT 2026-09-06** (interogare live `Prediction`, nu presupus). Textul anterior, datat 2026-07-28, **devenise fals în toate punctele care contau** și a fost citit ca atare într-o sesiune ulterioară: spunea „`model_champions` — 4 rânduri, toate `gate_validation_test` … niciun campion real", „`challengers` — 5 rânduri, toate `gate_validation_test` … zero challenger real activ", „`decision_feed` = 0", „`shadow_predictions` = 0". Feedback Loop-ul end-to-end pe care §5 punctul 3 îl declara blocat **s-a închis efectiv pe 23 august**. Starea reală de azi:

| Tabelă | Valoare | Observație |
|---|---:|---|
| `model_champions` | **5** (4 test + **1 real**) | `blend_v1/all`, `training_run_id=8ac89c70…`, promovat **2026-08-23 09:36**, `promoted_by=ADR-030-continuous-learning`, `superseded_at=NULL` — **primul campion real din istoria proiectului** |
| `challengers` — activi | **3** | 2 reali în `EVALUATING` (`xgboost_v1/all` din 5 aug, `blend_v1/all` din 24 aug) + 1 rând de test rămas în `SUCCEEDED` (`gate_validation_test/happy_path`, 14 iulie, altă pereche (familie, scop), fără efect asupra gărzii de consistență) |
| `decision_feed` | **1** | o singură decizie T3a, `committed` — promovarea de mai sus. Calea de aprobare umană a fost exercitată real, cap la cap, o dată |
| `challenger_evaluations` | **56** | verdicte zilnice, ambii Challengeri |
| `shadow_predictions` | **3675** | trafic real acumulat prin ADR-017 + ADR-050 + Challenger Shadow Batch (ADR-056) |
| `champion_health_evaluations` | **0** | neschimbat — `champion_guardian_enabled` absent din `model_config` → `False` |
| `recalibration_log` | **0** | neschimbat — `auto_recalibration_enabled` absent → `False`, deliberat (vezi §7) |

**Consecință nouă, care NU exista în iulie și e azi cel mai important fapt din această secțiune**: ambele sloturi de Challenger sunt ocupate de Challengeri blocați în verdict `monitoring`, iar `continuous_learning._process_pair()` rulează Faza B (antrenare nouă) **exclusiv** când nu există Challenger activ. Rezultat: **zero antrenări noi de la 24 august**, deși s-au acumulat 470 de meciuri terminate noi (prag: 30). Campionul care servește azi e antrenat pe 4 august, pe 49.983 de meciuri, față de 58.145 disponibile. Nu e defect — e absența unei reguli de ieșire din `monitoring`, prezisă în scris de **ADR-057** (PROPUS, 2026-08-17, neimplementat, nereferit de niciun alt document până azi). Vezi `BACKLOG_2026-09-06.md` §0.0.
...
2. **Activare `champion_guardian_enabled` (Etapa 2, monitorizare read-only)** — decizie deliberată, neexecutată; cod verificat safe/reversibil. **[ACTUALIZAT 2026-09-06]** Condiția pe care o aștepta („un Champion real de evaluat") **e îndeplinită din 23 august** — `blend_v1/all` e campion activ. Blocajul rămas e strict decizia de a porni flagul, nu absența unui subiect.
3. ~~**Feedback Loop end-to-end cu date reale**~~ — **ÎNCHIS 2026-08-23**, nu mai e „ce rămâne". Ciclul complet (predicție → rezultat real → shadow → evaluare → verdict `candidate_for_promotion` → propunere T3a → aprobare umană → promovare) a rulat efectiv, o dată, cap la cap, pentru `blend_v1`. Cifrele care motivau blocajul (`total_predictions=37`, `closed_loop_rows=0`) sunt depășite: 3675 de rânduri shadow, 56 de verdicte.
4. **Verificare proaspătă a `model_champions`/`challengers`** înainte de orice activare suplimentară de flag — Fazele A/B rulează zilnic, starea se poate schimba. **Confirmat util**: exact acest punct a fost ignorat între iulie și septembrie, iar §4 a rămas fals șase săptămâni.
5. **[NOU 2026-09-06] Decizia asupra ADR-057** (politica de expirare pentru Challenger-ul blocat în `monitoring`) — singurul lucru care ține azi antrenarea ML oprită. Vezi §4 și `BACKLOG_2026-09-06.md` §0.0.
