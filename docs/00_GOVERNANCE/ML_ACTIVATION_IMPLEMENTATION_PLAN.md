...
**Rezultat (executat 2026-08-03, Pasul 8)**: eliminarea simplă a termenului, testată izolat, **a degradat Oracle** pe backtest agregat (accuracy -0.29pp, Brier +0.0040, log-loss +0.0055, calibrare vizibil mai slabă) — declanșat explicit regula de oprire a proprietarului produsului. Analiză cauzală ulterioară a arătat că beneficiul empiric al bugului nu vine dintr-un semnal ascuns separat (un candidat de „componentă separată" testat explicit a performat mai slab), ci din faptul că `goals_weight=0.45` era subponderat pe tot spectrul de date. Un candidat de compensare (`goals_weight`→0.75) a fost testat și a arătat rezultate egale sau mai bune pe toate cele 5 metrici — **dar proprietarul produsului a decis explicit să NU aplice această recalibrare ca parte a acestui task**: bug fix-urile nu introduc schimbări de model; orice modificare a parametrilor matematici Oracle e un experiment de calibrare separat, cu propria aprobare, indiferent cât de favorabil arată un backtest. **Cod NEATINS** — bugul rămâne prezent, nerezolvat, documentat explicit ca „Oracle Insight" (`docs/03_ENGINE/ORACLE_INSIGHT_GOALS_WEIGHT.md`), cu analiza păstrată pentru un task viitor separat (FOLLOW-UP-P8-01 — Oracle Calibration). Regulă nouă permanentă adăugată în `CLAUDE.md`.
...
| 8 | **DEFERRED — cod neatins** — Fix dublă numărare `avg_goals_for` (§2.1, `ORACLE_INSIGHT_GOALS_WEIGHT.md`) | **Ridicat** — bug confirmat, afectează sistematic `off_stat` la fiecare predicție, cu o pondere invizibilă din config | **Mediu** — orice fix real schimbă rezultatul predicției | Ultimul, deliberat — vezi §6.1. Validare extinsă executată: backtest agregat (4849 meciuri, walk-forward) pe eliminarea simplă a arătat degradare → oprire, analiză cauzală, candidat de recalibrare `goals_weight` testat și validat prin backtest complet — dar **decizie explicită a proprietarului produsului: nu se aplică**, deoarece recalibrarea e o schimbare de model, nu un fix de bug. Follow-up separat: FOLLOW-UP-P8-01 — Oracle Calibration | Da, pentru orice implementare viitoare — schimbare de parametri matematici Oracle, experiment de calibrare separat, nu parte a acestui EPIC |
...
- **Discovery Rule** (regulă permanentă, formalizată în `CLAUDE.md` §„Disciplina ADR", 2026-08-03, precedent: Pasul 7, descoperirea `recalibration_log.new_dna_w`): dacă în timpul implementării unui ADR deja aprobat apare un contract/migrare/tabel/coloană/dependență nedescoperită la aprobare, implementarea se oprește, descoperirea e prezentată explicit, iar proprietarul produsului decide — rămâne în afara scopului, se extinde ADR-ul curent, sau se creează un ADR nou. Niciodată extindere tacită de scop.
