...
| `h2h_lookback_days` (config) | `DEFAULT_CONFIG` | Nefolosit direct în cascada DB curentă a H2H (`_build_h2h()`/`_h2h_record_from_history_rows()` nu-l citesc) | Verifică dacă mai e citit undeva în Sync Layer înainte de a-l elimina — NU confirmat cod mort 100%, doar suspect |
