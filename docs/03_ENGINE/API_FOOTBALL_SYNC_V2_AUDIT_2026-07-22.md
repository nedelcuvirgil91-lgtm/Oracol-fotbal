...
Runda inițială afirma greșit: *"cheia RapidAPI hardcodată... e duplicată de 3 ori — identic în `key_manager.py:24`... și `key_manager.py:31`... plus o a 4-a copie în `sync/poc_odds_api_romania_viability.py:47`"*. **Verificat direct (`grep` pe valoarea literală a cheii) în runda de completitudine**: linia `sync/poc_odds_api_romania_viability.py:47` conținea de fapt valoarea cheii `ODDS_API_KEY`, NU valoarea RapidAPI. Citarea anterioară era greșită — corectată în tabelul de mai sus. Imaginea reală, mai gravă decât cea raportată inițial: **toate cele 3 chei ale `oracle_api.py`** (nu doar RapidAPI) **erau duplicate** în tabelul de provideri din `key_manager.py`, plus Odds API mai avea o a treia copie într-un script POC.

**Corecție de securitate (R4.1, rundă ulterioară)**: valorile literale ale celor două chei citate mai sus în versiunea inițială a acestui paragraf (RapidAPI, Odds API) au fost eliminate din acest document — nu ar fi trebuit citate niciodată în plaintext într-un fișier de documentație, indiferent de scopul demonstrativ. Ambele chei au fost între timp eliminate și din cod (§ centralizare completă, toate cele 6 chei încărcate exclusiv din variabile de mediu, zero literali). Recomandare operațională: rotația ambelor chei (RapidAPI, Odds API) la providerii respectivi, fiindcă valorile au existat o vreme în istoricul git al acestui commit, chiar dacă repo-ul e privat.
