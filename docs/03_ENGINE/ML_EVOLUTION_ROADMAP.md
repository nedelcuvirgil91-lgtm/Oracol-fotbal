...
- **Status**: **Accepted** (V1_baseline, V2_damped — pe axa fidelitate ELO, susținut exclusiv de măsurători). **Formalizat**: `ADR-022-elo-margin-of-victory-v2-damped.md` — V2_damped ales, cod implementat în `ELOTracker.process_match()` (commit `631b0a4`), 7 teste noi. **Producția rămâne neatinsă** — Writer Protection blochează recalcularea implicită; activarea reală pe `match_history` (toate 53.409 rânduri — ELO afectează fiecare rând, nu doar cele 19.797 din P3.5) cere un Migration Plan separat, cu aprobare explicită. Execuția NU a continuat automat spre P4/P7.2 sau alt experiment.
