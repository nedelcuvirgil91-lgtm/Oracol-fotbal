...
def rollback_champion(
    ...
    league_scope)` — reactivează predecesorul campionului activ curent, dacă
    și numai dacă toate precondițiile structurale sunt satisfăcute. Niciodată
    nu ridică excepție către apelant — orice eșec (precondiție nesatisfăcută,
    Supabase indisponibil, cursă concurentă, eroare neașteptată) se întoarce
    ca `RollbackResult(status="rejected", reason=...)`.
    ...
    try:
        ...
        # Precondiție: campionul activ are un predecesor la care să revină.
        # get_champion_predecessor întoarce training_run_id-ul predecesorului
        # (rândul supersedat de campionul activ), sau None dacă nu există
        # campion activ ori nu are predecesor (primul campion) — R1.4.
