...
def test_odds_recent_results_step_declared_with_no_dependencies():
    """[ADAUGAT Sprint 2, Etapa C — Data Quality]"""
    names = {s.name: s for s in run_daily.PIPELINE_STEPS}
    assert "odds_recent_results" in names
    assert names["odds_recent_results"].depends_on == ()
...
def test_dry_run_includes_odds_recent_results_step_and_skips_it():
    """[ADAUGAT Sprint 2, Etapa C — Data Quality] dry_run sare sincronizarea,
    fara apel real la Odds API."""
    buf = io.StringIO()
    with contextlib.redirect_stdout(buf):
        run_daily.run(dry_run=True)
    out = buf.getvalue()
    assert "Sincronizare rezultate recente (Odds API)" in out
    section = out.split("Sincronizare rezultate recente (Odds API)")[1]
    assert "Sărit (dry run)" in section
