...
def test_run_calls_daily_sync_with_features_and_ml_skipped(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda leagues=None, limit_per_league=None, dry_run=False: 0,
    )
...
def test_run_calls_flashscore_foundation_data_layer(monkeypatch):
    ...
    monkeypatch.setattr(
        "providers.flashscore.run_foundation_data_layer.run",
        lambda leagues=None, limit_per_league=None, dry_run=False: fs_calls.append(
            (leagues, limit_per_league, dry_run)
        ) or 0,
    )
    result = live.run(dry_run=False)
    assert fs_calls == [(None, None, False)]
